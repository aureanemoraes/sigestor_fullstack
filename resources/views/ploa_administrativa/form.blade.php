...
<div>
  <p><span class="title">PLOA ADMINISTRATIVA</span> - <span class="subtitle-new" id="subtitle">Novo</span></p>
</div>
@php
    if(isset($ploa_administrativa)) {
      $route = route('ploa_administrativa.update', $ploa_administrativa->id);
    } else {
      $route = route('ploa_administrativa.store');
    }
@endphp
<form action="{{ $route }}" method="POST" id="form">
      ...
      <select class="form-control form-select @error('programa_id') is-invalid @enderror" id="programa_id" name="programa_id" aria-label="Selecione a instituição">
        ...
        @if(isset($programas))
          @foreach($programas as $programa)
            @if(isset($ploa_administrativa) && $programa->id == $plo->programa_id)
      ...
      <select class="form-control form-select @error('fonte_tipo_id') is-invalid @enderror" id="fonte_tipo_id" name="fonte_tipo_id" aria-label="Selecione a instituição">
        ...
        @if(isset($fontes))
          @foreach($fontes as $fonte)
            @if(isset($ploa_administrativa) && $fonte->id == $ploa_administrativa->fonte_tipo_id)
      ...
      <select class="form-control form-select @error('acao_tipo_id') is-invalid @enderror" id="acao_tipo" aria-label="Selecione a instituição">
        ...
        @if(isset($acoes))
          @foreach($acoes as $acao)
            @if(isset($ploa_administrativa) && $acao->id == $ploa_administrativa->acao_tipo_id)
  ...
  <div class="row">
    <input type="hidden" name="acao_tipo_id" id="acao_tipo_id" value="{{ isset($ploa_administrativa) ? $ploa_administrativa->acao_tipo_id : '' }}">

    <input type="hidden" name="exercicio_id" id="exercicio" value="{{ isset($ploa_administrativa) ? $ploa_administrativa->exercicio_id : '' }}">

    <input type="hidden" name="unidade_administrativa_id" id="unidade_administrativa" value="{{ isset($ploa_administrativa) ? $ploa_administrativa->unidade_administrativa_id : '' }}">
    ...
    <div class="form-group mb-3 col-4">
      ...
      <select class="form-control form-select @error('instituicao_id') is-invalid @enderror" id="instituicao_id" name="instituicao_id" aria-label="Selecione a instituição">
        @foreach($instituicoes as $instituicao)
          @if(isset($ploa_administrativa) && $instituicao->id == $ploa_administrativa->instituicao_id)
    ...
    <div class="form-group mb-3 col-4">
      ...
      <input type="number" class="form-control @error('valor') is-invalid @enderror" id="valor" name="valor" value="{{ isset($ploa_administrativa->valor) ? $ploa_administrativa->valor : null}}" placeholder="R$ 00,00">
      @error('valor')
        <div class="invalid-feedback">
          {{ $message }}
        </div>
