...
@if(isset($unidade_selecionada))
  ...
  @section('content')
    ...
    <section class="row">
      @if(session('error_ploa_administrativa') != null)
        <section class="alert-container">
          <div class="alert alert-danger alert-dismissible fade show" role="alert">
            <strong><i class="bi bi-x-circle-fill"></i> </strong>{{ session('error_ploa_administrativa') }}
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
          </div>
        </section>
        @php
          session()->forget(['error_ploa_administrativa'])
        @endphp
      @endif
    </section>
    ...
    <section class="row">
      ...
      <div class="collapse collpase-form" id="novoCollapse">
        <div class="card card-body">
          @include('ploa_administrativa.form')
        </div>
      </div>
    </section>
  ...
  @section('js')
    <script>
      ...
      $('#cancel-button').on('click', () => {
        ...
        $('#form').attr('action', `/ploa_administrativa`);
        $('#method').remove();
      ...
      $(document).ready(function() {
          ...
          $('#acao_tipo').select2();
          $('#ploa_administrativas').DataTable( {} );
          $('#unidade_administrativa_id').select2();
          $('#unidade_administrativa').val($('#unidade_administrativa_id').val());
