...
<section class="d-flex justify-content-end mb-2">
  <a href="{{ route('subnatureza_despesa.create') }}" type="button" class="btn btn-success">
    Novo
  </a>
</section>
<section>
    ...
    <table class="table" id="subnaturezas_despesas">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $subnatureza_despesa->natureza_despesa->nome }}</td>
          <td class="action-buttons">
            <form action="{{ route('subnatureza_despesa.destroy', $subnatureza_despesa->id) }}" method="post" id="form-delete">
              @csrf
              @method('delete')
              <div class="btn-group btn-group-sm" role="group" aria-label="acoes">
                <a type="button" href="{{ route('subnatureza_despesa.edit', $subnatureza_despesa->id) }}" class="btn btn-outline-warning" ><i class="bi bi-pen-fill"></i></a>
